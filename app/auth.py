...
from app.config import get_alchemy_config, get_jwt_secret
...
async def retrieve_user_from_token(token: Token, connection: ASGIConnection) -> User | None:
    ...
    if not user.is_active:
        return None
    return user
jwt_auth = JWTAuth[User](retrieve_user_handler=retrieve_user_from_token, token_secret=get_jwt_secret(), exclude=['/health', '^/$', '/scalar', '/openapi.json', '/api/auth/login', '/api/auth/register'])
